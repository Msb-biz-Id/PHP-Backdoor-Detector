--// Server UI Script - Position & Height Display (FIXED)
...
-- Safe function to get player GUI
local function getPlayerGui(player)
    ...
    local success, result = pcall(function()
        return player:WaitForChild("PlayerGui", 5) -- 5 second timeout
    end)
    
    if success and result then
        return result
    else
        warn("[Server Position UI] Failed to get PlayerGui for player: " .. tostring(player.Name))
        return nil
    end
end
...
local function isMobile(player)
    ...
    local player_gui = getPlayerGui(player)
    if not player_gui then return false end
    
    -- Check for mobile-specific services
    local success, result = pcall(function()
        return player_gui:FindFirstChild("TouchGui")
    end)
...
-- Create optimized UI for a player with error handling
local function createPlayerUI(player)
    if not player then
        warn("[Server Position UI] Cannot create UI for nil player")
        return nil
    end
    
    local player_gui = getPlayerGui(player)
    if not player_gui then
        warn("[Server Position UI] Cannot create UI - PlayerGui not found for: " .. tostring(player.Name))
        return nil
    end
    ...
    print("[Server Position UI] UI created for player: " .. tostring(player.Name))
    return main_frame
end

-- Create optimized player info frame
local function createPlayerInfoFrame(player_data, index, is_mobile)
    if not player_data or not ui_elements[player_data.name] then
        return nil
    end
    ...
    frame.Parent = ui_elements[player_data.name].player_list
...
-- Player joined with optimization and error handling
local function onPlayerAdded(player)
    if not player then
        warn("[Server Position UI] onPlayerAdded called with nil player")
        return
    end
    
    print("[Server Position UI] Player joined: " .. tostring(player.Name))
    
    player.CharacterAdded:Connect(function(character)
        if not character then
            warn("[Server Position UI] CharacterAdded with nil character for: " .. tostring(player.Name))
            return
        end
...
local function onPlayerRemoving(player)
    if not player then
        warn("[Server Position UI] onPlayerRemoving called with nil player")
        return
    end
    
    print("[Server Position UI] Player leaving: " .. tostring(player.Name))
...
local function initialize()
    print("[Server Position UI] Initializing fixed version...")
    ...
    if not success then
        warn("[Server Position UI] Error connecting player events: " .. tostring(error_msg))
        return
    end
    
    -- Create UI for existing players
    local existing_players = Players:GetPlayers()
    for _, player in ipairs(existing_players) do
        if player then
    ...
    task.spawn(function()
        while true do
            local success, error_msg = pcall(updateAllUIs)
            if not success then
                warn("[Server Position UI] Error in update loop: " .. tostring(error_msg))
            end
            wait(CONFIG.update_interval)
        end
    end)
    
    print("[Server Position UI] Fixed system initialized!")
...
if not success then
    warn("[Server Position UI] Failed to initialize: " .. tostring(error_msg))
end
