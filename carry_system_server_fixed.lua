...
-- FIXED: Proper cleanup function that restores movement smoothly
local function clearState(shared)
	...
	-- restore target movement and networking - FIXED VERSION
	if shared.target then
		...
		if tHum then
			...
			tHum.WalkSpeed = 16 -- Restore normal walk speed
			tHum.JumpPower = 50 -- Restore normal jump power
			
			-- Change state to restore movement
			pcall(function()
		...
		if tHrp then
			-- CRITICAL FIX: Don't change network ownership back to nil immediately
			-- This was causing the movement delay issue
			-- Instead, let the client handle it naturally
			
			-- Clear velocities smoothly
			tHrp.AssemblyLinearVelocity = Vector3.zero
			tHrp.AssemblyAngularVelocity = Vector3.zero
			
			-- Small delay before restoring network ownership to prevent lag
			task.wait(0.1)
			pcall(function()
...
local function beginCarry(carrier, target, animKey)
	...
	-- FIXED: Better target physics handling
	...
	-- Smooth movement update using RunService
	local connection
	connection = RunService.Heartbeat:Connect(function()
		...
		if tHrp and bodyVelocity and bodyVelocity.Parent then
			...
			bodyVelocity.Velocity = direction * 10 -- Smooth movement multiplier
		end
	end)
